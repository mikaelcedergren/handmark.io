...
export function routeForSourcePath(file) {
  return file.startsWith('src/') || file.startsWith('public/') ? '/' : null;
}
...
export function classifyChanges(
  ...
  const behaviorInterfacePattern = /^src\/app\/app\.component\.(?:ts|html)$/u;
  const e2ePattern =
    /^(?:tests\/e2e\/|playwright\.config\.mjs$|scripts\/(?:run-e2e|e2e-server)\.mjs$)/u;
  const highRiskPattern =
    /^(?:AGENTS\.md|CLAUDE\.md|\.agents\/|\.codex\/|\.gitignore$|package\.json|pnpm-lock\.yaml|pnpm-workspace\.yaml|angular\.json|tsconfig(?:\.[^.]+)?\.json|cx-product\.json|\.github\/|\.env\.web\.example$|server\/|launchd\/|bin\/|ops\/|src\/main\.server\.ts$|src\/app\/app\.config\.server\.ts$|scripts\/verify-change\.mjs$|tests\/(?:server\/|change-verification\.test\.mjs$)|DEVELOPMENT-VERIFICATION\.md$)/u;
  const interfaceFiles = changedFiles.filter(
    (file) =>
      interfacePattern.test(file) &&
      !/^(?:src\/main\.server\.ts$|src\/app\/app\.config\.server\.ts$)/u.test(file),
  );
  const behaviorInterfaceChange = has(behaviorInterfacePattern);
  ...
  if (highRisk) {
    checks.add('full');
    if (e2eChange || behaviorInterfaceChange) checks.add('e2e');
    ...
    if (interfaceFiles.length > 0) {
      ...
      if (behaviorInterfaceChange) checks.add('e2e');
      checks.add('visual');
      routes.push(...interfaceFiles.map(routeForSourcePath));
    }
    if (e2eChange) {
...
function plannedChecks(classification) {
  const checks = {
    full: commandCheck('full', 'Full Handmark proof', 0, ['corepack', 'pnpm', 'check'], {
      hostAccess: true,
    }),
    ...
    visual: {
      hostAccess: true,
      id: 'visual',
      label: `Rendered Handmark check (${classification.routes.join(', ')})`,
      phase: 2,
      routes: classification.routes.map((route) => ({
        origin: 'http://127.0.0.1:4230',
        route,
      })),
      type: 'visual',
    },
...
export function checkOwnsPath(checkId, file) {
  ...
  if (checkId === 'e2e') {
    return /^(?:src\/|public\/|server\/|tests\/e2e\/|scripts\/(?:run-e2e|e2e-server)|playwright\.config|package\.json|pnpm-lock\.yaml)/u.test(
      file,
    );
  }
  return false;
...
export function checkInputHash(check, snapshot) {
  const hash = createHash('sha256');
  hash.update(
    `handmark-change-verification-v1\0${process.version}\0${check.id}\0${JSON.stringify(check.command ?? check.routes ?? [])}`,
  );
...
async function runVisual(check) {
  ...
  try {
    ...
    for (const target of check.routes) {
      ...
      let response = await page.goto(`${target.origin}${target.route}`, {
        timeout: 20_000,
        waitUntil: 'networkidle',
      });
      if (!response?.ok()) throw new Error(`Route ${target.route} returned ${response?.status()}`);
      if (new URL(page.url()).pathname === '/login') {
        await page.getByLabel('Access password').fill('handmark-local-development-password');
        await page.getByRole('button', { name: 'Enter Handmark' }).click();
        await page.waitForLoadState('networkidle');
        response = await page.goto(`${target.origin}${target.route}`, {
          timeout: 20_000,
          waitUntil: 'networkidle',
        });
        if (!response?.ok()) {
          throw new Error(`Authenticated route ${target.route} returned ${response?.status()}`);
        }
      }
...
function printPlan(classification, changedFiles, checks, reusable) {
  console.log('Handmark change verification');
