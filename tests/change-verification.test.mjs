...
test('documentation, interface, E2E, and high-risk paths select their owning proofs', () => {
  ...
  const ui = classifyChanges(['src/styles/site.scss']);
  assert.deepEqual(ui.checks, ['format', 'typecheck', 'build-browser', 'visual']);
  assert.deepEqual(ui.routes, ['/']);
  assert.deepEqual(classifyChanges(['src/app/app.component.ts']).checks, [
    'format',
    'typecheck',
    'build-browser',
    'e2e',
  ...
  assert.deepEqual(classifyChanges(['tests/e2e/handmark.spec.cjs']).checks, ['format', 'e2e']);
  for (const file of [
    'AGENTS.md',
    'package.json',
    '.env.web.example',
    'server/src/application-repository.ts',
    'launchd/com.handmark.server.plist',
    'scripts/verify-change.mjs',
    'tests/change-verification.test.mjs',
  ]) {
    assert.deepEqual(classifyChanges([file]).checks, ['full'], file);
  }
  assert.deepEqual(classifyChanges(['package.json', 'src/styles/site.scss']).checks, [
    'full',
    'visual',
  ]);
  assert.deepEqual(classifyChanges(['package.json', 'src/app/app.component.ts']).checks, [
    'full',
    'e2e',
    'visual',
  ]);
...
test('route mapping is product-owned', () => {
  assert.equal(routeForSourcePath('src/app/app.component.ts'), '/');
  assert.equal(routeForSourcePath('public/assets/handmark-logo.svg'), '/');
  assert.equal(routeForSourcePath('src/styles.scss'), '/');
  assert.equal(safeEvidenceName('/'), 'home');
});
...
test('source digests include content, executable mode, and symbolic-link target', (context) => {
  const root = mkdtempSync(path.join(os.tmpdir(), 'handmark-verification-'));
  context.after(() => rmSync(root, { force: true, recursive: true }));
...
test('check reuse hashes only change when an owned input changes', () => {
  ...
  const original = checkInputHash(check, {
    'README.md': 'docs-a',
    'src/app/app.component.ts': 'source-a',
  });
  ...
  const sourceChange = checkInputHash(check, {
    'README.md': 'docs-b',
    'src/app/app.component.ts': 'source-b',
  });
...
test('receipts are atomic, private, and invalid receipts are never trusted', (context) => {
  const root = mkdtempSync(path.join(os.tmpdir(), 'handmark-receipt-'));
  context.after(() => rmSync(root, { force: true, recursive: true }));
